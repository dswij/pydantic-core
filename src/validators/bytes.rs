...
impl BuildValidator for BytesValidator {
    ...
    fn build(
        ...
        let use_constrained = schema.get_item("max_length").is_some()
            ...
            || match config {
                Some(config) => {
                        config.get_item("bytes_max_length").is_some()
                        || config.get_item("bytes_min_length").is_some()
                }
                None => false,
            };
        if use_constrained {
...
impl Validator for BytesValidator {
    ...
    fn validate_strict<'s, 'data>(
        ...
        Ok(input.strict_bytes()?.into_py(py))
    }
...
impl BytesConstrainedValidator {
    ...
    fn _validation_logic<'s, 'data>(
        ...
        ValResult::Ok(py_bytes.into_py(py))
    }
}
